...
class CouponTestCase(TestCase):
    def test_generate_code(self):
        self.assertIsNotNone(re.match(f"^[{CODE_CHARS}]{{{CODE_LENGTH}}}", Coupon.generate_code()))
    ...
    def test_generate_code_segmented(self):
        ...
        self.assertIsNotNone(
            re.match(
                f"^([{CODE_CHARS}]{{{SEGMENT_LENGTH}}}{SEGMENT_SEPARATOR}){{{num_segments}}}[{CODE_CHARS}]{{{num_rest}}}$",
